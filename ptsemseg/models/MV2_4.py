...
# MV2_4 change resnet 7*7 to three 3*3
class MV2_4_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(MV2_4_ResNet, self).__init__()
        self.inplanes = 64*2

        self.convbnrelu1_1 = conv2DBatchNormRelu(in_channels=3, k_size=3, n_filters=64, padding=1, stride=2, bias=False)
        self.convbnrelu1_2 = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=64, padding=1, stride=1,
                                                 bias=False)
        self.convbnrelu1_3 = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=128, padding=1, stride=1,
                                                 bias=False)
...
def MV2_4_ResNet18(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_4_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '18_imagenet'
        url = models_urls[key]
...
def MV2_4_ResNet34(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_4_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '34_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_4_ResNet50(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_4_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '50_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_4_ResNet101(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_4_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '101_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_4_ResNet152(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_4_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '152_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
