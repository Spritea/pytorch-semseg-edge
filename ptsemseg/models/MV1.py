...
class MV1_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(MV1_ResNet, self).__init__()
        self.inplanes = 64
        ...
        # only for >=res50
        self.c7 = nn.Conv2d(2048, 512, kernel_size=7, stride=1, padding=3, bias=False)
        self.c5 = nn.Conv2d(2048, 512, kernel_size=5, stride=1, padding=2, bias=False)
        self.c3 = nn.Conv2d(2048, 512, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x_l4c5 = self.c5(l4)
        x_l4c3 = self.c3(l4)
        x_l4_final = torch.cat([x_l4c7, x_l4c5, x_l4c3], dim=1)
...
def MV1_ResNet18(num_classes,pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV1_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs,num_classes=num_classes)
    if pretrained:
        key = '18_imagenet'
        url = models_urls[key]
...
def MV1_ResNet34(num_classes,pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV1_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs,num_classes=num_classes)
    if pretrained:
        key = '34_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV1_ResNet50(num_classes,pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV1_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs,num_classes=num_classes)
    if pretrained:
        key = '50_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV1_ResNet101(num_classes,pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV1_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs,num_classes=num_classes)
    if pretrained:
        key = '101_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV1_ResNet152(num_classes,pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV1_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs,num_classes=num_classes)
    if pretrained:
        key = '152_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
