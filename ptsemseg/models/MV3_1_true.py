...
class GAU(nn.Module):
    ...
    def forward(self, input_small, input_big):
        ...
        high_size = input_big.size()[2:]
        # low channel usually > high channel
        # if self.in_size != self.out_size:
        #     input_low = self.conv(input_low)
        upsample_small = F.upsample(input_small, high_size, mode='bilinear')
        input_cat = torch.cat([upsample_small, input_big], dim=1)
        ...
        multiply=gp*input_big
        out=multiply+upsample_small
        return out
...
# MV3_1 similar with PAN's CAB module
class MV3_1_true_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(MV3_1_true_ResNet, self).__init__()
...
def MV3_1_true_ResNet18(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV3_1_true_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '18_imagenet'
        url = models_urls[key]
...
def MV3_1_true_ResNet34(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV3_1_true_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '34_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV3_1_true_ResNet50(num_classes, pretrained=True, **kwargs):
    """Constructs a MV1_ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV3_1_true_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '50_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV3_1_true_ResNet101(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV3_1_true_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '101_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV3_1_true_ResNet152(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV3_1_true_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '152_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
